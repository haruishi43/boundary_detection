...
class BaseDataset(data.Dataset):
    ...
    def _val_sync_transform(self, img, mask):
        ...
        pad_index = torch.ones(
            1, crop_size, crop_size
        )  # keep validation the same with training (for loss calculation)
    ...
    def _sync_transform(self, img, mask):
        ...
        if self.scale:
            short_size = random.randint(
                int(self.base_size * 0.75), int(self.base_size * 2.0)
            )
        else:
        ...
        if short_size < crop_size:
            ...
            mask = ImageOps.expand(
                mask, border=(0, 0, padw, padh), fill=(255, 255, 255)
            )  # pad 255 for ground truth
